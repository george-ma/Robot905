 ...
 *
 */
public class TestFighterDefensive extends FighterRobot {
	...
	public TestFighterDefensive(City c, int a, int s, Direction d, int id, int health) {
...
//		System.out.println("fightID: " + fightID);
		...
		String decideMove = determineMove(avenueFinal,streetFinal,fightID,energy);
	...
	private double calcPriority(OppData opponent) {
		double priority;
		priority = (opponent.getHealth()/100)*2+(calculateDist(opponent)/32)*5;
		return priority;
	}
